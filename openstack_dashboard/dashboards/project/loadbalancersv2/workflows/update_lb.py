...
class UpdateLBDetailsAction(SetLBDetailsAction):
    address = forms.CharField(widget=forms.widgets.Input(
    ...
    port = forms.IntegerField(widget=forms.widgets.Input(
        attrs={'readonly': 'readonly'}),
                              label=_("LB Port"),
                              required=False,
                              min_value=1,
...
class UpdateInstancesAction(SelectInstancesAction):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        pre_selected = []
        try:
            pre_selected = args[0]['selected_members']
        except Exception:
            exceptions.handle(request, err_msg)
        self.fields[self.get_member_field_name('member')].initial = pre_selected
