...
#
#    Author: Vivek Jain <vivekjain@ebaysf.com>
#
    ...
    def clean(self):
        ...
        if not self.is_update:
            ...
            if 'address' in data \
                ...
                selected_ip_port_combo = '%s:%s' %(address.split(':')[0],selected_lb_port)
                if selected_ip_port_combo in ipPortCombo:
                    raise forms.ValidationError(
                            _('Requested IP and port combination already exists %s '
...
class SelectInstancesAction(workflows.MembershipAction):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        all_instances = []
        try:
            all_instances, has_more_data = api.nova.server_list(request)
        except:
        ...
        self.fields[self.get_member_field_name('member')].choices = available_instances
...
class SelectMonitorAction(workflows.Action):
    ...
    interval = forms.IntegerField(label=_("Health Check Interval (in seconds)"),
                                  required=False,
                                  min_value=1,
                                  max_value=600,
    ...
    timeout = forms.IntegerField(label=_("Retry count before markdown"),
                                  ...
                                  min_value=1,
                                  max_value=100,
                                  help_text=_("Number of times health check should "
                                              "be attempted before marking down a member"))
