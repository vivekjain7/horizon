...
from .tabs import LoadBalancerDetailTabs
from .tables import LoadBalancersTable
from .workflows import LaunchLoadBalancer, UpdateLoadBalancer
...
class UpdateView(workflows.WorkflowView):
    ...
    def get_object(self, *args, **kwargs):
        if not hasattr(self, "_object"):
            ...
            print "loadbalancer_id"
            print loadbalancer_id
            try:
                vip = api.lbaasv2.show_loadbalancer(self.request , loadbalancer_id)
                self._object = vip.readable(self.request)
            except Exception as e:
                redirect = reverse("horizon:project:loadbalancersv2:index")
                msg = _('Unable to retrieve load balancer details. %s' % e.message)
...
class DetailView(tabs.TabView):
    ...
    def get_data(self):
        if not hasattr(self, "_loadbalancer"):
            try:
                loadbalancer_id = self.kwargs['loadbalancer_id']
                loadbalancer = api.lbaasv2.show_loadbalancer(self.request, loadbalancer_id)
            except Exception as e:
                redirect = reverse('horizon:project:loadbalancersv2:index')
                exceptions.handle(self.request,
