...
class LBDetails(neutron.NeutronAPIDictWrapper):
    ...
    def __init__(self, vip, listener=None, pool=None, members=None, monitors=None,
                 profile_name=None, cert=None, key=None, chain=None):
        vip['pool'] = pool
    ...
    def readable(self, request=None):
        ...
        #     try:
        #         if self.profile_name.upper() == _construct_common_cert_profile_name(request).upper():
        ...
        if self.pool is not None:
            try:
                ...
                #status_string = '%s \n pool: %s' % (pFormatted['status'], pool['status'].lower())
                ...
                if pool.monitors is not None:
                    try:
                        for m in pool.monitors:
                            ...
                            #status_string = '%s \n monitor: %s' % (pFormatted['status'], monitor_status.lower())
                            #pFormatted['status'] = status_string
